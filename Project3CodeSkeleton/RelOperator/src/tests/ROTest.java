...
class ROTest extends TestDriver {
	...
	protected boolean test2() {
		try {
			...
			// test sort-merge join operator
			saveCounts(null);
			SortMergeJoin join = new SortMergeJoin(new FileScan(s_drivers, drivers),
	...
	protected boolean test3() {
		try {
			...
			// sort-merge join of sort-merge join; selection for output's sake
			saveCounts(null);
			SortMergeJoin join1 = new SortMergeJoin(new FileScan(s_groups, groups),
					new FileScan(s_rides, rides), 0, 1);
			SortMergeJoin join2 = new SortMergeJoin(join1, new IndexScan(s_drivers, ixdrivers,
					drivers), 2, 0);
